...
unsigned int buffer[MAX_WIDTH * MAX_HEIGHT];						// main buffer
unsigned int* out = buffer;											// out array

// data to pass into the kernel
...
int main(int argc, char* argv[])
{
	...
	for (int i = 0; i < times; i++)
	{
		...
		program = clLoadSource(context, "Stage1/Render.cl", &err);
		if (err != CL_SUCCESS) {
	...
	// output BMP file
	write_bmp(outputFilename, out, width, height, width);
}
